...
browser.runtime.onStartup.addListener(() => {
    ...
    browser.storage.local.clear(); // Clear storage on startup to prevent stale entries
...
async function initializeTabs() {
    ...
    tabs.forEach(tab => {
        ...
        if (!tab.url.startsWith('chrome://')) {
            ...
            browser.storage.local.set({[tab.id]: tab.url}); // Save each tab's URL and ID
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (!tab.url.startsWith('chrome://')) {
        ...
        if (changeInfo.url) {
            ...
            browser.storage.local.set({[tabId]: changeInfo.url}); // Update storage with new URL
...
browser.tabs.onRemoved.addListener((tabId, removeInfo, tab) => {
...
async function injectModulesBasedOnURL(tabId, url, TabReload = false, LogMessage) {
    ...
    if (TabReload) {
        ...
        try {
            const response = await fetch(browser.runtime.getURL('/modules/module_list.json'));
            if (!response.ok) {
...
function CheckModuleInject(moduleName, tabId) {
    return new Promise((resolve, reject) => {
        browser.tabs.sendMessage(tabId, { type: "ping", moduleName: moduleName }, response => {
